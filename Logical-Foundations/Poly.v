...
Definition filter_even_gt7 (l : list nat) : list nat :=
  filter (fun n => evenb n && negb (blt_nat n 7)) l.
...
Lemma map_r : forall (X Y : Type) (f : X -> Y) (l : list X) (x : X),
...
Proof.
  intros. induction l.
  - reflexivity.
  - simpl. rewrite <- IHl. rewrite -> map_r. reflexivity.
Qed.
(** [] *)
...
Example fold_example4 :
  fold (fun n b => andb (evenb n) b)  [4; 6; 7; 2] true = false.
Proof. reflexivity. Qed.
(** [] *)
...
Definition plus (n m : nat) : nat :=
  fun (X : Type) (f:X -> X) x => n X f (m X f x).
...
Definition mult (n m : nat) : nat :=
  fun (X : Type) (f:X -> X) x => n X (m X f) x.
